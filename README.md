# skala-vue
SKALA Vue 실습 프로젝트 — 여러 도시의 실시간 날씨를 검색/조회하고, 지도 또는 카드 형태로 한눈에 볼 수 있는 날씨 대시보드
실습 단계 별로 commit message를 자세히 남겨뒀습니다. 각 step대로 요구사항을 다 이해하고 구현한 뒤, 차별점 관련 코드는 따로 구성해뒀다가 추가하는 방식으로 실습을 진행했습니다.
